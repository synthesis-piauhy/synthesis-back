...
def activity_created(actor: User, report: ActivityReport) -> None:
    ...
    notify(
        actor,
        kind="conclusao",
        message=f'Relato "{report.title}" enviado com sucesso.',
        href=href,
        key=f"activity:created:{report.id}",
    )
    for editor in User.objects.filter(role__in=(UserRole.EDITOR, UserRole.ADMIN), active=True).iterator():
        notify(
            editor,
            kind="atividade",
            message=f'{actor.name} enviou o relato "{report.title}".',
            href=href,
            key=f"activity:received:{report.id}",
        )
...
def activity_updated(actor: User, report: ActivityReport) -> None:
    notify(
        actor,
        kind="conclusao",
        message=f'Alterações no relato "{report.title}" salvas.',
        href=f"/relatos/{report.id}",
...
def cycle_opened(actor: User, cycle: WeeklyCycle) -> None:
    ...
    notify(
        actor,
        kind="conclusao",
        message=f'Ciclo "{cycle.label}" aberto com sucesso.',
        href="/ciclos",
        key=f"cycle:created:actor:{suffix}",
    )
    notify_managers(
        kind="ciclo",
        message=f'O ciclo "{cycle.label}" foi aberto. Envie seu relato até {deadline_label(cycle.deadline)}.',
        href="/relatos/novo",
        key=f"cycle:opened:{suffix}",
    )
...
def cycle_closed(actor: User, cycle: WeeklyCycle) -> None:
    ...
    notify(
        actor,
        kind="conclusao",
        message=f'Ciclo "{cycle.label}" encerrado.',
        href="/ciclos",
        key=f"cycle:closed:actor:{suffix}",
    )
    notify_managers(
        kind="ciclo",
        message=f'O ciclo "{cycle.label}" foi encerrado.',
        href="/relatos",
        key=f"cycle:closed:{suffix}",
    )
...
def cycle_deadline_updated(actor: User, cycle: WeeklyCycle) -> None:
    ...
    notify_managers(
        ...
        href="/relatos/novo",
        key=f"cycle:deadline:{suffix}",
    )
    notify(
        actor,
        kind="conclusao",
        message=f'Prazo do ciclo "{cycle.label}" atualizado.',
        href="/ciclos",
        key=f"cycle:deadline:actor:{suffix}",
    )
...
def cycle_reopened(actor: User, cycle: WeeklyCycle) -> None:
    ...
    notify(
        actor,
        kind="conclusao",
        message=f'Ciclo "{cycle.label}" reaberto.',
        href="/ciclos",
        key=f"cycle:reopened:actor:{suffix}",
    )
    notify_managers(
        kind="ciclo",
        message=f'O ciclo "{cycle.label}" foi reaberto. Novo prazo: {deadline_label(cycle.deadline)}.',
        href="/relatos/novo",
        key=f"cycle:reopened:{suffix}",
    )
...
def draft_generated(actor: User, report: WeeklyReport) -> None:
    notify(
        actor,
        kind="conclusao",
        message=f'Rascunho do relatório de "{report.cycle.label}" gerado.',
        href="/relatorio-semanal",
        key=f"report:draft:{report.id}",
    )
...
def pdf_generated(actor: User, version: ReportVersion) -> None:
    notify(
        ...
        kind="conclusao",
        message=(
            f'PDF do relatório de "{version.weekly_report.cycle.label}" (versão {version.version}) gerado.'
        ),
        href="/historico",
        key=f"report:pdf:{version.id}",
    )
...
def ensure_deadline_reminders(user: User) -> None:
    ...
    for cycle in cycles:
        if (
            ...
            and not ActivityReport.objects.filter(cycle=cycle, manager=user).exists()
        ):
        ...
        if user.role == UserRole.MANAGER:
            ...
            pending = (
                ...
                .exclude(id__in=ActivityReport.objects.filter(cycle=cycle).values("manager_id"))
                .count()
            )
            if not pending:
                continue
